    ...
    public int savePayment(PaymentDTO paymentDTO, String userId, String tokenStripe)
        ...
        if (paymentDTO.getType().equals(Constants.THANH_TOAN_TIEN_MAT)) {
            ...
            CompletableFuture.runAsync(() -> {
                try {
                    mailService.sendEmailWhenBookingIsSuccess(Constants.MAIL_SENDER, "hoanhuudev@gmail.com",
                            Constants.NOTIFICATION_BOOK_TOUR_SUCCESS, customer.getName(), booking, "123/link");
            ...
            if (tokenStripe != null) {
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        ...
                        mailService.sendEmailWhenBookingIsSuccess(Constants.MAIL_SENDER, "hoanhuudev@gmail.com",
                                Constants.NOTIFICATION_BOOK_TOUR_SUCCESS, customer.getName(), bookingSave, "123/link");
                    } catch (MessagingException e) {
                        e.printStackTrace();
                    }
                });
